...
#[derive(Clone, Default)]
pub struct StargazeSwapFactoryQuerier {
    pairs: HashMap<String, PairInfo>,
}

impl StargazeSwapFactoryQuerier {
    pub fn new(pairs: &[(&String, &PairInfo)]) -> Self {
        StargazeSwapFactoryQuerier {
...
struct DefaultQueryHandler {
    ...
    sg_swap_factory_querier: StargazeSwapFactoryQuerier,
}
        ...
        match &request {
            QueryRequest::Wasm(WasmQuery::Smart {
                ...
                FactoryQueryMsg::Pair { asset_infos } => {
                    ...
                    match self.sg_swap_factory_querier.pairs.get(&key) {
...
impl WasmMockQuerier {
    pub fn new(base: MockQuerier<Empty>) -> Self {
        WasmMockQuerier {
            query_handler: DefaultQueryHandler {
                base,
                sg_swap_factory_querier: StargazeSwapFactoryQuerier::default(),
            },
            cw20_query_handler: CW20QueryHandler {
    ...
    // Configure the Stargaze pair
    pub fn with_sg_swap_pairs(&mut self, pairs: &[(&String, &PairInfo)]) {
        self.query_handler.sg_swap_factory_querier = StargazeSwapFactoryQuerier::new(pairs);
    }
