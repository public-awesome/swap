use sg_swap::asset::{Asset, AssetInfo, AssetInfoExt, AssetInfoValidated};
use sg_swap::factory::{
...
use sg_swap::querier::query_token_balance;
use sg_swap_pair_stable::math::{MAX_AMP, MAX_AMP_CHANGE, MIN_AMP_CHANGING_TIME};
...
fn store_pair_code(app: &mut App) -> u64 {
    let pair_contract = Box::new(
        ContractWrapper::new_with_empty(
            sg_swap_pair_stable::contract::execute,
            sg_swap_pair_stable::contract::instantiate,
            sg_swap_pair_stable::contract::query,
        )
        .with_reply_empty(sg_swap_pair_stable::contract::reply),
    );
